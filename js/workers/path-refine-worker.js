...
function applyOff(rHelio, t, policy, tMid, tDep, exaggerateSun = true) {
  let tOff = t;
  if (policy === 'mid_epoch') tOff = tMid;
  if (policy === 'locked_departure') tOff = tDep;
  if (policy === 'none') return { x: rHelio.x, y: rHelio.y, z: rHelio.z };
  const off = getSunBarycentricOffset(tOff, !!exaggerateSun);
  return { x: rHelio.x + off.x, y: rHelio.y + off.y, z: rHelio.z + off.z };
}

function densify(orb, tDep, tof, offsetPolicy, n0, maxN, exaggerateSun = true) {
  ...
  for (let i = 0; i <= n0; i++) {
    ...
    const s = applyOff(h, tDep + dt, offsetPolicy, tMid, tDep, exaggerateSun);
    pts.push({ t_sec: tDep + dt, ...s, r_helio: h });
  }
  // Mild densify — aggressive thresholds created jags / dual-looking strokes
  const eps = 0.05;
  const Lmax = 0.35;
  let cur = pts;
  for (let pass = 0; pass < 4 && cur.length < maxN; pass++) {
    const next = [cur[0]];
    let added = 0;
      ...
      if (chord > Lmax || chord > eps) {
        ...
        if (st) {
          ...
          if (err > eps * 0.5 || chord > Lmax) {
            const s = applyOff(h, tM, offsetPolicy, tMid, tDep, exaggerateSun);
  ...
  // Sort by time so densify inserts never reverse order (jags)
  cur.sort((a, b) => a.t_sec - b.t_sec);
  return cur;
}
...
self.onmessage = (ev) => {
  ...
  const {
    requestId, orbit, tDep, tof, offsetPolicy, nSamples, maxSamples, exaggerateSun,
  } = msg;
  if (!orbit || !(tof > 0)) {
