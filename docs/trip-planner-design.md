...
HELIOS is a browser-first interplanetary mission visualizer with a real physics core: JPL Approximate Positions ephemeris (`js/physics/kepler.js`), a bracketed universal-variable Lambert solver (`js/physics/lambert.js`), multi-leg gravity-assist routing (`js/physics/routing.js` + `js/physics/gravity-assist.js`), porkchop C3/V∞ heatmaps (`js/ui/porkchop.js`), patched-conic mission budgets (`js/physics/mission-budget.js`), and a cinematic Three.js scene. It already supports planet and major-moon origin/destination, automatic nearest-feasible-window snap, flyby date optimization, mission animation, and JSON plan **export** (`exportMissionPlan` in `js/ui/route-display.js`; no import UI yet).
...
| **UI** | Fat modules: `route-display.js` (~29 KB), `porkchop.js` (~14 KB), `route-planner.js` (~12 KB); `index.html` ~837 lines (CSS/DOM/shell + importmap) |
