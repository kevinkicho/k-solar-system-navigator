// Measures the porkchop coverage uplift from trying short- AND long-way
// Lambert branches instead of only the prograde-heuristic branch.
//
// Prints, for each origin→destination pair, the number of cells solved by:
//   heuristic  — current app behavior (single branch from crossY sign)
//   both       — whichever of short/long is cheaper (new behavior)
// …plus the min Δv each found, so we can confirm "both" never regresses.
...
// longWay: null = heuristic, true/false = forced branch.
...
function sweep(b1, b2, departStart, years, modeBoth){
  const GX=65, GY=52;
  ...
  for (let iy=0; iy<GY; iy++){
    for (let ix=0; ix<GX; ix++){
      ...
      let bestDv = NaN;
      const branches = modeBoth ? [false, true] : [null];
      for (const lw of branches){
        const s = lambert(r1v, r2v, tof, MU, lw);
        if (!s) continue;
        const dv = mag(sub(s.v1,vb1)) + mag(sub(s.v2,vb2));
        if (isNaN(bestDv) || dv < bestDv) bestDv = dv;
      }
      if (!isNaN(bestDv)){
        solved++;
        if (bestDv < minDv){ minDv=bestDv; minDep=dep; minTof=tof; }
      }
    }
  }
  return { cells: GX*GY, solved, minDv, minDep, minTof };
}

const today = toSim(new Date(Date.UTC(2026,3,22)));
const pairs = [['Earth','Mars'],['Earth','Venus'],['Earth','Jupiter'],['Mars','Jupiter'],['Venus','Mars']];

console.log('Porkchop coverage: heuristic (old) vs both-branch (new)');
console.log('─'.repeat(88));
for (const [o,d] of pairs){
  const b1=body(o), b2=body(d);
  const heur = sweep(b1, b2, today, 5, false);
  const both = sweep(b1, b2, today, 5, true);
  const depH = heur.minDep!==undefined ? fromSim(heur.minDep).toISOString().slice(0,10) : '—';
  const depB = both.minDep!==undefined ? fromSim(both.minDep).toISOString().slice(0,10) : '—';
  const gain = both.solved - heur.solved;
  const pct = (100*gain/heur.solved).toFixed(1);
  console.log(
    `${o}→${d.padEnd(8)} ` +
    `heuristic: ${String(heur.solved).padStart(4)}/${heur.cells} cells, ` +
    `min Δv=${(heur.minDv/1000).toFixed(2)} km/s  |  ` +
    `both:  ${String(both.solved).padStart(4)}/${both.cells} cells  (+${gain}, +${pct}%), ` +
    `min Δv=${(both.minDv/1000).toFixed(2)} km/s`
  );
}
